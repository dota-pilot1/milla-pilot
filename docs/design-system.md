...
**레이어 대비 (구분감)**
- **텍스트가 놓이는 면은 흰색**이다 — 카드·사이드바·헤더(`--card`/`--sidebar`/`--background`).
  가독성이 먼저다. 이 면들을 브랜드 색으로 칠해 구분을 만들지 않는다.
- 구분은 **페이지 바닥(`--page`)과 보더**가 담당한다.
  상거래 화면이라 **바닥을 밝게 두고(`--page` 0.968) 구분은 보더(`--border` 0.872)+`shadow-sm`가 책임진다.**
  바닥을 어둡게 눌러 구분을 만들면 그 위에 놓인 타이틀·설명이 흐려 보인다.
  이 조합을 바꿀 때는 **바닥을 어둡게 하는 대신 보더를 진하게** 하는 방향을 먼저 쓴다.
- **페이지 바닥 위에는 `text-muted-foreground`를 쓰지 않는다.** muted 는 흰 카드 안 보조 텍스트 전용이고,
  바닥 위 설명 문구는 `text-foreground/75`로 한 단 올린다(`shared/ui/PageHeader` 참고).
...
- 보더는 카드가 배경에 묻히지 않을 만큼 유지한다. 현재 라이트 `oklch(0.885)`.
